...
1. **Job directory management.** One job = one person = one output folder,
   with a `.tmp/` scratch area inside it. build.py creates it; nothing writes
   real profile data anywhere else.
...
MANIFEST_SCHEMA = "visitor-profile-builder-py/manifest@1"
TMP_DIRNAME = ".tmp"
...
def slugify(name, fallback="profile"):
...
class Manifest:
    ...
    @classmethod
    def create(cls, job_dir, profile, source=None):
        job_dir = os.path.abspath(job_dir)
        os.makedirs(os.path.join(job_dir, TMP_DIRNAME), exist_ok=True)
        data = {
            "schema": MANIFEST_SCHEMA,
            "job_id": f"{slugify(profile.get('name'))}-{now_stamp()}",
            "created_at": now_iso(),
            "job_dir": job_dir,
            "profile": {
...
def _cmd_list(args):
    ...
    print(f"person : {m.data['profile'].get('name')}")
    if not arts:
