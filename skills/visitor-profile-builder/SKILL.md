...
`assets/profile.example.json`, and that example rendered out to
`assets/profile.example.html` so you can see the target layout without
running anything). **Never hand-edit the HTML or xlsx** — edit the JSON and
regenerate, so the outputs can't drift apart.
